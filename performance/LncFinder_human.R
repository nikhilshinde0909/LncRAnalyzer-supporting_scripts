...
LncFinder_Human <- LncFinder_Human %>% group_by(Subset) %>% arrange(Subset)

LncFinder_Human1 <- LncFinder_Human %>% group_by(Subset,Class,Label) %>% summarise(Count=n())
write.table(LncFinder_Human1, 'LncFinder_human_summary.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)
...
LncFinder_Human_metrics <- LncFinder_Human %>%
  ...
  ungroup()
write.table(LncFinder_Human_metrics, 'LncFinder_human_metrics.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)

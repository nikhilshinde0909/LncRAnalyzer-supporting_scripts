...
CPAT_Plants <- CPAT_Plants %>% group_by(Subset) %>% arrange(Subset)

CPAT_Plants1 <- CPAT_Plants %>% group_by(Subset,Class,Label) %>% summarise(Count=n())
write.table(CPAT_Plants1, 'CPAT_plants_summary.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)
...
CPAT_Plants_metrics <- CPAT_Plants %>%
  ...
  ungroup()
write.table(CPAT_Plants_metrics, 'CPAT_plants_metrics.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)

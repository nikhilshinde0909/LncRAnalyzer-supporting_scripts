...
FEELnc_arabidopsis <- list(True_labels,FEELnc_arabidopsis) %>% reduce(inner_join)
FEELnc_arabidopsis <- FEELnc_arabidopsis %>% group_by(Subset) %>% arrange(Subset)

FEELnc_arabidopsis1 <- FEELnc_arabidopsis %>% group_by(Subset,Class,Label) %>% summarise(Count=n())
write.table(FEELnc_arabidopsis1, 'FEELnc_arabidopsis_summary.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)
...
FEELnc_arabidopsis_metrics <- FEELnc_arabidopsis %>%
  group_by(Subset) %>%
  do(calculate_metrics(.)) %>%
  ungroup()
write.table(FEELnc_arabidopsis_metrics, 'FEELnc_arabidopsis_metrics.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)

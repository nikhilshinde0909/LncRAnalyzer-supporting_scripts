...
CPAT_Human <- list(True_labels,CPAT_Human) %>% reduce(inner_join)
CPAT_Human <- CPAT_Human %>% group_by(Subset) %>% arrange(Subset)

CPAT_Human1 <- CPAT_Human %>% group_by(Subset,Class,Label) %>% summarise(Count=n())
write.table(CPAT_Human1, 'CPAT_human_summary.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)
...
CPAT_Human_metrics <- CPAT_Human %>%
  group_by(Subset) %>%
  do(calculate_metrics(.)) %>%
  ungroup()
write.table(CPAT_Human_metrics, 'CPAT_human_metrics.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)

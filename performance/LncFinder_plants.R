...
LncFinder_Plants <- list(True_labels,LncFinder_Plants) %>% reduce(inner_join)
LncFinder_Plants <- LncFinder_Plants %>% group_by(Subset) %>% arrange(Subset)

LncFinder_Plants1 <- LncFinder_Plants %>% group_by(Subset,Class,Label) %>% summarise(Count=n())
write.table(LncFinder_Plants1, 'LncFinder_plants_summary.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)
...
LncFinder_Plants_metrics <- LncFinder_Plants %>%
  group_by(Subset) %>%
  do(calculate_metrics(.)) %>%
  ungroup()
write.table(LncFinder_Plants_metrics, 'LncFinder_plants_metrics.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)

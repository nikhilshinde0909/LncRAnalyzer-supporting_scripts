...
CPAT_Sorghum <- CPAT_Sorghum %>% group_by(Subset) %>% arrange(Subset)

CPAT_Sorghum1 <- CPAT_Sorghum %>% group_by(Subset,Class,Label) %>% summarise(Count=n())
write.table(CPAT_Sorghum1, 'CPAT_sorghum_summary.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)
...
CPAT_Sorghum_metrics <- CPAT_Sorghum %>%
  ...
  ungroup()
write.table(CPAT_Sorghum_metrics, 'CPAT_sorghum_metrics.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)

...
RNAsamba_Sorghum <- list(True_labels,RNAsamba_Sorghum) %>% reduce(inner_join)
RNAsamba_Sorghum <- RNAsamba_Sorghum %>% group_by(Subset) %>% arrange(Subset)

RNAsamba_Sorghum1 <- RNAsamba_Sorghum %>% group_by(Subset,Class,Label) %>% summarise(Count=n())
write.table(RNAsamba_Sorghum1, 'RNAsamba_sorghum_summary.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)
...
RNAsamba_Sorghum_metrics <- RNAsamba_Sorghum %>%
  group_by(Subset) %>%
  do(calculate_metrics(.)) %>%
  ungroup()
write.table(RNAsamba_Sorghum_metrics, 'RNAsamba_sorghum_metrics.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)

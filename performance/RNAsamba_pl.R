...
RNAsamba_pl <- RNAsamba_pl %>% group_by(Subset) %>% arrange(Subset)

RNAsamba_pl1 <- RNAsamba_pl %>% group_by(Subset,Class,Label) %>% summarise(Count=n())
write.table(RNAsamba_pl1, 'RNAsamba_pl_summary.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)
...
RNAsamba_pl_metrics <- RNAsamba_pl %>%
  ...
  ungroup()
write.table(RNAsamba_pl_metrics, 'RNAsamba_pl_metrics.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)

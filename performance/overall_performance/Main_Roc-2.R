...
library(dplyr)
library(stringr)
data <- read.table('ROC_values6.TSV', header = T, sep = '\t')
...
p <- data %>% ggplot(aes(x = FPR, y = TPR, group = Approach, colour = Approach)) +
  ...
  facet_wrap(facets = ~ Approach, ncol = 3) +
...
pdf('Lnc_ROC_common.pdf', width = 15, height = 24)
p
dev.off()
...
pdf('Lnc_ROC_common1.pdf', width = 10, height = 8)
p1
dev.off()
...
pdf('Lnc_ROC_common.pdf', width = 24, height = 15)
p2
dev.off()
...
pdf('Lnc_ROC_common3.pdf', width = 12.5, height = 10)
p3
dev.off()

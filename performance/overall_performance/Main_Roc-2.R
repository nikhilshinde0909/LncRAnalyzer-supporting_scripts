...
library(dplyr)
library(stringr)
data <- read.table('ROC_values5.TSV', header = T, sep = '\t')
...
pdf('Lnc_ROC_tools_smooth.pdf', width = 10, height = 8)
p1
dev.off()
...
pdf('Lnc_ROC_approaches_smooth.pdf', width = 24, height = 15)
p2
dev.off()
...
pdf('Lnc_ROC_all_smooth.pdf', width = 12.5, height = 10)
p3
dev.off()

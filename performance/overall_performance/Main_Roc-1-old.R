...
library(dplyr)
data <- read.table('ROC_values4.TSV', header = T, sep = '\t')
sigmoid <- function(x, k = 1) {
  return(1 / (1 + exp(-k * (x - 0.5))))
}

data <- data %>% group_by(Approach) %>% mutate(FPR=sigmoid(x = FPR,k=10),
                                               TPR=sigmoid(TPR,k = 10))
approach <- c("CPAT-Sorghum", "CPAT-Plants", "CPAT-Human",
              ...
              "CPC2", "LGC", "PfamScan",
              "CPPred-Sorghum","CPPred-Plants", "CPPred-Human**")
...
pdf('Lnc_ROC_common1.pdf', width = 7, height = 5)
p
dev.off()

...
CPC2 <- list(True_labels,CPC2) %>% reduce(inner_join)
CPC2 <- CPC2 %>% group_by(Subset) %>% arrange(Subset)

CPC21 <- CPC2 %>% group_by(Subset,Class,Label) %>% summarise(Count=n())
write.table(CPC21, 'CPC2_summary.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)
...
CPC2_metrics <- CPC2 %>%
  ...
  ungroup()
write.table(CPC2_metrics, 'CPC2_metrics.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)

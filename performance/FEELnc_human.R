...
FEELnc_human <- FEELnc_human %>% group_by(Subset) %>% arrange(Subset)

FEELnc_human1 <- FEELnc_human %>% group_by(Subset,Class,Label) %>% summarise(Count=n())
write.table(FEELnc_human1, 'FEELnc_human_summary.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)
...
FEELnc_human_metrics <- FEELnc_human %>%
  ...
  ungroup()
write.table(FEELnc_human_metrics, 'FEELnc_human_metrics.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)

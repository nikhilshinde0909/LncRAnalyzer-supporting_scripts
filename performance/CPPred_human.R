...
CPPred_human <- list(True_labels,CPPred_human) %>% reduce(inner_join)
CPPred_human <- CPPred_human %>% group_by(Subset) %>% arrange(Subset)

CPPred_human1 <- CPPred_human %>% group_by(Subset,Class,Label) %>% summarise(Count=n())
write.table(CPPred_human1, 'CPPred_human_summary.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)
...
CPPred_human_metrics <- CPPred_human %>%
  group_by(Subset) %>%
  do(calculate_metrics(.)) %>%
  ungroup()
write.table(CPPred_human_metrics, 'CPPred_human_metrics.TSV', row.names = F, col.names = T,
            sep = '\t', quote = F)

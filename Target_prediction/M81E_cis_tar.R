...
library(dplyr)
library(tidyverse)
Cis1 <- read.table('M81E_Cis_C1+.TSV', header = T, sep = '\t')
Cis2 <- read.table('M81E_Cis_C1-.TSV', header = T, sep = '\t')
...
write.table(data,'M81E_Cis_targets.TSV', row.names = F, col.names = T, sep = '\t', quote = F)

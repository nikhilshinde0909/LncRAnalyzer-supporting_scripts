...
library(dplyr)
library(tidyverse)
Trans1 <- read.table('Roma_Trans_C1+.TSV', header = T, sep = '\t')
Trans2 <- read.table('Roma_Trans_C1-.TSV', header = T, sep = '\t')
...
write.table(data,'Roma_Trans_targets.TSV', row.names = F, col.names = T, sep = '\t', quote = F)
